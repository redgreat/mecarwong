...
#define PWM_MAX_VALUE 255  /* Arduino PWM范围是0-255 */

/* PWM引脚定义 */
static const int pwm_channels[4] = { 2, 3, 5, 6 }; /* 对应Arduino的PWM引脚 */
...
static void set_motor(int idx, float duty, bool dir)
{
    ...
    int pwm_value = (int)(duty * PWM_MAX_VALUE);
    ...
    /* 设置方向引脚 */
    digitalWrite(dir_pins[idx][0], dir ? HIGH : LOW);
    digitalWrite(dir_pins[idx][1], dir ? LOW : HIGH);
}
...
{
    Serial.begin(115200);
    Serial.println("Initializing Mecanum RoboCar");
    ...
    Serial.println("Mecanum RoboCar initialized successfully");
}
...
void move_forward_left(float speed)
{
    ...
    float component = speed / sqrt(2.0f);  /* 对角线分量 */
...
void move_s_path(float speed, float amplitude)
{
    ...
    float omega = amplitude * sin(phase);
